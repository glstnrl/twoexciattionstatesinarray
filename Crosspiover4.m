...
for teta=0:0.01:pi
    for phi=0:0.01:2*pi
        posS=[R*sin(teta)*cos(phi),R*sin(teta)*sin(phi),R*cos(teta)];
            ...
            E1=((sin(teta1)*cos(-d1*k))/d1);
            B1=((sin(teta1)*cos(-d1*k))/d1);
        ...
        for d=0:0.4*lamda:lamda*2
            posS=[R*sin(teta)*cos(phi),R*sin(teta)*sin(phi),R*cos(teta)];
            absR1=posS-[d/(2*sqrt(2)),0,d/(2*sqrt(2))];
            absR2=posS-[-d/(2*sqrt(2)),0,-d/(2*sqrt(2))];
            d1=sqrt(sum(absR1.^2));
            d2=sqrt(sum(absR2.^2));
            teta1=acos(absR1(3)/d1);
            ...
            E1=((sin(teta1)*cos(-d1*k))/d1);
            E2=((sin(teta2)*cos(-d2*k))/d2);
            B1=((sin(teta1)*cos(-d1*k))/d1);
            B2=((sin(teta2)*cos(-d2*k))/d2);
            E1vec=[E1*cos(teta1)*cos(phi1),E1*cos(teta1)*sin(phi1),-E1*sin(teta1)];
...
title('Destructive interference points');
grid on;